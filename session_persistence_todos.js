  ...
  (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if (!created) {
        ...
        store.createNewTodoList(todoListTitle);
...
app.post("/lists/:todoListId/complete_all", (req, res, next) => {
  let todoListId = req.params.todoListId;
  let completeAll = res.locals.store.completeAllTodos(+todoListId);
  ...
  (req, res, next) => {
    ...
    if (!todoList) {
      ...
      if (!errors.isEmpty()) {
        ...
        let created = res.locals.store.createNewTodo(+todoListId, todoTitle);
        if (!created) {
          next(new Error("Not found."));
        } else {
